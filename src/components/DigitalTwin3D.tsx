...
export default function DigitalTwin3D({
  ...
  const propsRef = useRef({ temperature, vibration, speed, wear, healthScore, selectedComponent });
  useEffect(() => {
    propsRef.current = { temperature, vibration, speed, wear, healthScore, selectedComponent };
  }, [temperature, vibration, speed, wear, healthScore, selectedComponent]);
  ...
  useEffect(() => {
    ...
    const materials: Record<string, THREE.MeshStandardMaterial> = {
      defaultMetal: new THREE.MeshStandardMaterial({ color: 0x3e4451, roughness: 0.3, metalness: 0.8 }),
      brightMetal: new THREE.MeshStandardMaterial({ color: 0x8e929d, roughness: 0.15, metalness: 0.95 }),
      darkChassis: new THREE.MeshStandardMaterial({ color: 0x1b1b1b, roughness: 0.4, metalness: 0.7 }),
      ...
      hydraulics: new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.2, metalness: 0.7 }),
      copper: new THREE.MeshStandardMaterial({ color: 0xb45309, roughness: 0.3, metalness: 0.8 }),
      activeHighlight: new THREE.MeshStandardMaterial({ color: 0x22c55e, emissive: 0x15803d, roughness: 0.2, metalness: 0.6 }),
      warningHighlight: new THREE.MeshStandardMaterial({ color: 0xf59e0b, emissive: 0xb45309, roughness: 0.2, metalness: 0.6 }),
      criticalHighlight: new THREE.MeshStandardMaterial({ color: 0xef4444, emissive: 0xb91c1c, roughness: 0.2, metalness: 0.6 })
    };
    ...
    // Buttons array indicators (small keyboard block)
    ...
    // Workpiece Block being milled (Steel block)
    ...
    // ==========================================
    // 9. SPINDLE assembly (Moves vertical Y and depth Z)
    // ==========================================
    const spindleAssemblyGroup = new THREE.Group();
    spindleAssemblyGroup.position.set(0.6, 0.6, 0);

    // Spindle outer cylinder head housing (non-rotating)
    ...
    // Inner rotating shaft group (rotates around vertical Y axis)
    ...
    // Cutting Tool (Component 5)
    // Create a spiral cylinder drill tip
    const drillGeo = new THREE.CylinderGeometry(0.08, 0.01, 0.5, 12);
    const drillMesh = new THREE.Mesh(drillGeo, materials.copper); // Copper color represents coating
    ...
    // ==========================================
    // 10. COOLANT SYSTEM & PARTICLE NOZZLE
    // ==========================================
    const coolantAssemblyGroup = new THREE.Group();
    coolantAssemblyGroup.position.set(0.6, 0.6, 0); // positioned relative to spindle head
    ...
    // Direct flexible coolant nozzle pipes
    ...
    // Interactive Drag to Rotate and Scroll to Zoom
    ...
    const handleCanvasClick = (e: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        if (obj && obj.userData.componentId && onSelectComponent) {
    ...
    const animate = () => {
      ...
      // Retrieve live parameter values from synchronized ref
      const current = propsRef.current;

      // 1. Spindle Rotation (RPM speeds: normal 1500 -> 2800)
      const rotSpeedRad = (current.speed / 60) * Math.PI * 2; // radians/second
      rotatingSpindleGroup.rotation.y += rotSpeedRad * delta;

      // 2. Linear axis oscillation to simulate actual multi-axis machining tool path
      const amplitudeX = 0.4;
      ...
      // Servo Y/Z slide carriage kinematics
      const amplitudeY = 0.15;
      ...
      // Depth Axis Z feed
      const currentOffsetZ = Math.sin(time * 0.4) * 0.2;
      spindleAssemblyGroup.position.z = currentOffsetZ;
      coolantAssemblyGroup.position.z = currentOffsetZ;

      // Servo rotor detail rotations
      motorX.rotation.z += (currentOffsetX * 5) * delta;
      motorY.rotation.z += (currentOffsetY * 5) * delta;

      // 3. Coolant Flow Particles Animation
      ...
      particles.forEach((p, idx) => {
        ...
        if (p.life > 1.0) {
          ...
          // Dripping gravity and spray velocity pointing downwards towards block
          p.velocity.set(
            (Math.random() - 0.5) * 0.2,
            -1.8 - Math.random() * 0.5,
            -0.3 - Math.random() * 0.3
        ...
        // Shrink particle as it drips/dries
        p.mesh.scale.setScalar(1.0 - p.life * 0.8);
      });

      // 4. Heat overlay glow & pointlight when temperature climbs
      // Nominal temp 25°C -> critical thermal threshold 45°C
      const normTemp = Math.max(0, Math.min(1, (current.temperature - 25) / 20));
      thermalLight.intensity = normTemp * 1.5;

      // 5. Tool wear visual coloring (Shiny steel 12min -> worn copper-bronze 180min -> black scorched failed)
      const normWear = Math.max(0, Math.min(1, current.wear / 200));
      const targetWearColor = new THREE.Color(0xb45309); // bronze
      if (normWear > 0.8) {
        materials.copper.color.setHex(0x1e1e1e); // scorched dark
      } else {
        materials.copper.color.lerpColors(new THREE.Color(0x8e929d), targetWearColor, normWear);
      }

      // 6. Shake/Tremor vibration based on index (vibration mm/s)
      const vibAmp = Math.max(0, (current.vibration / 10) * 0.08);
      if (vibAmp > 0) {
        cncGroup.position.set(
          Math.sin(time * 65) * vibAmp,
          Math.cos(time * 70) * vibAmp,
          Math.sin(time * 75) * vibAmp
        );
      } else {
        cncGroup.position.set(0, 0, 0);
      }

      // 7. Dynamic component state highlights and blinks
      // First, reset all materials to nominal baseline
      Object.keys(componentMeshes).forEach(cid => {
        const componentList = componentMeshes[cid];
        const isSel = cid === current.selectedComponent;
        
        // Decide appropriate material
        ...
        // Apply highlighting or warning overlays
        componentList.forEach(m => {
          if (isSel) {
            // Apply flashing warning if health is low
      ...
      // Slow automated rotate when not actively dragging
      if (!isDragging) {
        cncGroup.rotation.y += 0.08 * delta;
      }
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative min-h-[300px] flex items-center justify-center">
      ...
      {/* 3D UI Control Hint Overlay */}
      <div className="absolute bottom-3 right-3 bg-black/75 border border-slate-800 rounded-lg px-2.5 py-1 text-[8px] font-mono text-slate-400 pointer-events-none select-none flex items-center gap-1.5 uppercase tracking-wider">
